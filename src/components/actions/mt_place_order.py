...
class MtPlaceOrder(Action):
    ...
    def __place_order(self, magic, symbol, order_type, volume, price=None, deviation=20, tp=None, sl=None, tp_rel=None, sl_rel=None):
        if not mt5.symbol_select(symbol, True):
            logger.error(f"Symbol {symbol} not found")
            return None

        if magic is None or not isinstance(magic, int):
            logger.error("Magic number is missing")
            return None        
